...
import frs.minidraw.demo.puzzle.PuzzlePieceTool.SqureSlots;
  ...
  public static void main(String[] args) {
    DrawingEditor editor = 
      new MiniDrawApplication( "Put the pieces into place",
                               new PuzzleFactory() );
    editor.open();
    editor.setTool( new PuzzlePieceTool(editor) );
    ...
    BoardActionTool bat = new BoardActionTool(editor);
...
class PuzzleFactory implements Factory {
  ...
  public DrawingView createDrawingView( DrawingEditor editor ) {
    ...
    System.out.println("StdViewWithBackground");
    
    ((StdViewWithBackground) view).setLayout(new GridLayout(3, 3));
    
    SqureSlots[] slot = SqureSlots.values();

    for (int i = 0; i < 9; i++) {
		System.out.println("this is being done");

		JPanel piecePanel = new JPanel();
		
		piecePanel.setName(slot[i].toString());
		System.out.println(slot[i]);
		
		((StdViewWithBackground) view).add(piecePanel);
	}
    return view;
  }
  ...
  public JTextField createStatusField( DrawingEditor editor ) {
	  System.out.println("createStatusField" );
    return null;
  }
}
